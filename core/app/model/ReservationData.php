...
class ReservationData
{
	...
	public function add()
	{
		$sql = "insert into reservation (no,title,note,medic_id,date_at,time_at,pacient_id,user_id,price,status_id,payment_id,sick,symtoms,medicaments,created_at, quirur, descrip, observ, detall) ";
		$sql .= "value (\"$this->no\",\"$this->title\",\"$this->note\",\"$this->medic_id\",\"$this->date_at\",\"$this->time_at\",$this->pacient_id,$this->user_id,\"$this->price\",$this->status_id,$this->payment_id,\"$this->sick\",\"$this->symtoms\",\"$this->medicaments\",$this->created_at,\"$this->quirur\",\"$this->descripcion\",\"$this->observ\",\"$this->detall\")";
...
//todo asignar a esta tabla las citas nada mas con valor de "3" en quirur
	public static function getAll()
	{
		$sql = "select * from " . self::$tablename . " where quirur= 3 order by date_at";
		$query = Executor::doit($sql);
		return Model::many($query[0], new ReservationData());
	}
	public static function getAll2()
	{
		...
		$sql = "select * from " . self::$tablename . " where quirur= $quirur order by date_at";
		$query = Executor::doit($sql);
		return Model::many($query[0], new ReservationData());
	}
